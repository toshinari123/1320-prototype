...
export default function Home() {
  ...
  const submitMessage = async (e) => {
    ...
    console.log(message);
    let res = await createConversation({ user_id: auth.id, room_id: room.id, message: message });
    if (res == null) {
  ...
  //dont comment on below code please
  const getout = () => {
    ...
    <div>
      ...
      <div className={`${!auth && 'hidden'} bg-gradient-to-b from-orange-400 to-rose-400 h-screen p-12`}>
        <main className='flex w-full max-w-[1020px] h-[700px] mx-auto bg-[#FAF9FE] rounded-[25px] backdrop-opacity-30 opacity-95'>
          {room === null && !isFilter && !isMakeRoom && (<div className='bg-[#F0EEF5] w-[325px] h-[700px] rounded-l-[25px] p-4 overflow-auto relative'>
            <ChatList onChatChange={updateMessages} userId={auth.id} />
            <div className="overflow-hidden space-y-3">
              <button onClick={showFilter} className='text-xs w-full max-w-[295px] p-3 rounded-[10px] bg-violet-200 font-semibold text-violet-600 text-center bottom-4'>FILTER</button>
              <button onClick={showMakeRoom} className='text-xs w-full max-w-[295px] p-3 rounded-[10px] bg-violet-200 font-semibold text-violet-600 text-center bottom-4'>MAKE ROOM</button>
              <button onClick={signOut} className='text-xs w-full max-w-[295px] p-3 rounded-[10px] bg-violet-200 font-semibold text-violet-600 text-center bottom-4'>LOG OUT</button>
            </div>
          </div>)}
          ...
          {isFilter && (<section className='rounded-r-[25px] w-full max-w-[690px] grid grid-rows-[80px_minmax(450px,_1fr)_65px]'>
            <div className='rounded-tr-[25px] w-ful'>
              <div className='flex gap-3 p-3 items-center'>
                ...
                <p className='font-semibold text-gray-600 text-base'>FILTER (TODO)</p>
              </div>
              <hr className='bg-[#F0EEF5]' />
            </div>
          </section>)}
          {isMakeRoom && (<section className='rounded-r-[25px] w-full max-w-[690px] grid grid-rows-[80px_minmax(450px,_1fr)_65px]'>
            <div className='rounded-tr-[25px] w-ful'>
              <div className='flex gap-3 p-3 items-center'>
                <form onSubmit={getout}>
                  <button type='submit' className='bg-violet-500 rounded-full py-2 px-6 font-semibold text-white text-sm'>&lt;-</button>
