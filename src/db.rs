...
pub fn find_user_by_phone(
    ...
    use crate::schema::users::dsl::*;
    let user = users.filter(phone.eq(user_phone)).first::<User>(conn).optional()?;
    Ok(user)
}
...
pub fn insert_new_conversation(
    ...
    println!("inserting conversation...");
    println!("{}", format!("{:?}", new_conversation));
...
pub fn get_all_rooms(conn: &mut SqliteConnection) -> Result<Vec<RoomResponse>, DbError> {
    ...
    for room in &data {
        ...
        for id in user_ids.to_vec() { ids.insert(id.to_string()); }
        rooms_map.insert(room.id.to_string(), user_ids.to_vec());
    }
    let ids = ids.into_iter().collect::<Vec<_>>();
    let users_data: Vec<User> = users::table.filter(users::id.eq_any(ids)).get_results(conn)?;
    let users_map: HashMap<String, User> = HashMap::from_iter(users_data.into_iter().map(|item| (item.id.to_string(), item)));
